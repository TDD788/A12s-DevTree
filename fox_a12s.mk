# MKBOOTIMG
$(shell chmod a+x device/samsung/a12s/prebuilt/mkboot/mkbootimg)
...
define export_build_vars
    @echo "$(GREEN)Exporting build vars from the a12s tree$(RESET)"
...
define set_env_var
    @echo "$(RED_BACK)Environment Variable CURR_DEVICE not set... Aborting$(RESET)"
...
define var_not_eq
    @echo "$(RED_BACK)CURR_DEVICE not equal to a12s$(RESET)"
    @echo "$(RED_BACK)CURR_DEVICE = $(CURR_DEVICE)$(RESET)"
    @echo "$(RED)If this is a mistake, then export CURR_DEVICE to the correct codename$(RESET)"
    @echo "$(RED)Skipping a12s specific build vars...$(RESET)"
endef
